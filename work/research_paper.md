...
## Ranking Future Content Decline Risk from Search Performance Signals

**Author:** Dania Yasir  
**Project:** FlyRank ML Internship Capstone  
**Status:** Working research-paper manuscript  

> This manuscript is the polished research-paper source. The current repository is the source of truth for final metrics and methodology. The abstract and external references will be finalized after the body is reviewed.

---

## Abstract

*To be finalized after the full manuscript review. The final abstract will use five sentences: question → data → method → headline result → intended use.*
...
Recent work on tabular learning under temporal shift shows that model performance can depend strongly on how training and validation data are split, reinforcing the need to treat the validation design as part of the research claim rather than as a minor implementation detail [1]. Broader tabular distribution-shift benchmarks similarly show that performance can change when the deployment distribution differs from the training distribution [2]. These findings motivate the use of client-disjoint validation in this study instead of relying on random-row cross-validation.
...
The more demanding comparison is against the momentum-only Random Forest. Here, the full model improves Precision@100 by **8.4 percentage points**. This smaller gap suggests that recent momentum carries a large share of the predictive signal, while the additional trend-shape, traffic, CTR, position, and client-relative features provide useful incremental ranking information.

![Final validation performance](figures/01_validation_performance.png)

**Takeaway:** The full model produced the strongest observed client-disjoint Precision@100, while the smaller gap to the momentum-only Random Forest shows that much of the signal is already present in recent momentum.
...
Client-disjoint evaluation makes heterogeneity visible. In the final action-playbook summary, **24 of 34 clients** show positive concentration of decline among their high-risk pages. The **median client uplift is +18.1 percentage points** relative to each client's overall decline rate.
...
A ranking metric alone does not explain where the model fails. Out-of-fold error analysis helps define the boundary of the prediction problem.
...
3. [`w05_model_training!.ipynb`](notebooks/w05_model_training!.ipynb) — learned-model development
4. [`w06_validation_audit.ipynb`](notebooks/w06_validation_audit.ipynb) — honest validation and leakage audit
5. [`w07_action_playbook.ipynb`](notebooks/w07_action_playbook.ipynb) — final queue, metrics, figures, and action policy
6. [`capstone.ipynb`](notebooks/capstone.ipynb) — concise capstone narrative
...
1. Cai, H., & Ye, H.-J. (2025). **Understanding the Limits of Deep Tabular Methods with Temporal Shift.** *Proceedings of the 42nd International Conference on Machine Learning (ICML), PMLR 267*, 6366–6386. https://proceedings.mlr.press/v267/cai25j.html

2. Gardner, J., Popović, Z., & Schmidt, L. (2023). **Benchmarking Distribution Shift in Tabular Data with TableShift.** *Advances in Neural Information Processing Systems (NeurIPS), Datasets and Benchmarks Track*. https://openreview.net/forum?id=gej8U5p2or

3. Heuton, K., Muench, F., Shrestha, S., Stopka, T. J., & Hughes, M. C. (2025). **Decision-aware Training of Spatiotemporal Forecasting Models to Select a Top-K Subset of Sites for Intervention.** *Proceedings of the 42nd International Conference on Machine Learning (ICML), PMLR 267*, 23136–23154. https://proceedings.mlr.press/v267/heuton25a.html
